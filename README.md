...
若最初的“这个……”请求以 `自动审批` 结尾，本轮会启用自动审批模式。路由 AI 能明确匹配插件时会自动进入该插件；无法判断插件时才向主人询问。规划 AI 每返回一条 CLI 命令，程序都会再发起一次独立 AI 审查，检查命令是否符合原始要求、参数是否有可靠依据，以及执行是否安全。审查通过时机器人会发送“AI自动审批并执行”并立即执行；未通过或审查失败时只把该条命令交给主人手动确认。无论是否启用自动审批，任务最终完成仍必须由主人确认后才会退出并释放历史。
